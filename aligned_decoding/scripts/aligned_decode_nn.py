"""Cross-patient aligned speech decoding using SVM with neural network-style pipeline.

Runs phoneme classification with optional cross-patient pooling, CCA
alignment, and joint PCA dimensionality reduction. Uses a bagging SVM
classifier evaluated over stratified k-fold cross-validation.
"""
...
def init_parser():
    """Create argument parser for aligned neural-network-style decoding.
...
def aligned_decoding():
    """Run the aligned decoding pipeline with joint PCA and CCA.

    Parses CLI arguments, loads patient neural data, applies optional
    cross-patient pooling with CCA alignment or joint PCA, trains an SVM
    classifier via stratified k-fold cross-validation over multiple
    iterations, and saves accuracy results to disk.
    """
    parser = init_parser()
    ...
    for _ in range(n_iter):
        ...
        for train_idx, test_idx in cv.split(D_tar, lab_tar):
            ...
            # TODO replace classifer with neural network
