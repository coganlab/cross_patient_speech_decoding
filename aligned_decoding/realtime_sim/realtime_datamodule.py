    ...
    def setup(self, stage=None):
        ...
        # reduce data to latent space if pooling across patients
            if self.pool:
                ...
                val_data = tgt_pca.transform(val_data)
                test_data = tgt_pca.transform(self.test_data)
                if self.train_data_cross is not None:
                    for j in range(len(self.train_data_cross)):
                        data_red, _ = reduce_to_latent_space(
                                        self.train_data_cross[j]
                                    )
                        self.train_data_cross[j] = data_red
                # align data if specified
                if self.align:
                    # only need to align cross patients to target space,
                    # no modification of target patient data
                    for j in range(len(self.train_data_cross)):
                        self.train_data_cross[j] = align_to_target(
                                                        self.aligner,
                                                        train_data_tgt,
                                                        self.train_data_cross[j],
                                                        train_labels_tgt,
                                                        self.train_labels_cross[j]
                                                    )
                else:
                    min_dim = min(train_data_tgt.shape[-1],
                                  min([d.shape[-1] for d in self.train_data_cross]))
                    ...
                    for j in range(len(self.train_data_cross)):
                        self.train_data_cross[j] = self.train_data_cross[j][:, :, :min_dim]
...
class CTCHeldOutTargetValCVDataModule(CTCHeldOutDataModule):
    """
...
class CTCHeldOutTargetValAlignCVDataModule(CTCHeldOutDataModule):
    """
...
def reduce_to_latent_space(data, dim_red=PCA, n_components=0.9):
    shapes = data.shape
    data_2d = data.reshape(-1, shapes[-1])

    dr = dim_red(n_components=n_components)
    data_r = dr.fit_transform(data_2d)
    data_r = data_r.reshape(shapes[0], shapes[1], -1)
...
def align_to_target(aligner, target_data, source_data, target_labels, source_labels):
    ...
    source_2d = source_data.reshape(-1, source_shapes[-1])
    
    target_shapes = target_data.shape
    target_2d = target_data.reshape(-1, target_shapes[-1])

    align = aligner()
    align.fit(target_2d, source_2d, target_labels.numpy(),
              source_labels.numpy())
    source_a = align.transform(source_2d)
    source_a = source_a.reshape(source_shapes[0], source_shapes[1], -1)
