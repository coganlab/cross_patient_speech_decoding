...
def _normal(X):
    ...
    return ((X - np.mean(X, axis=0)) @
            np.linalg.inv(sqrtm(np.cov(X, rowvar=False))))
...
def _sinkhorn(p, q, X, Y, gamma, maxiter):
    """Entropy-Regularized (Sinkhorn) Optimal Transport between
    distributions"""
    x2, y2 = np.sum(np.power(X, 2), axis=0), np.sum(np.power(Y, 2), axis=0)
    C = (np.tile(y2[np.newaxis, :], (X.shape[1], 1)) +
         np.tile(x2[:, np.newaxis], (1, Y.shape[1])) - 2 * (X.T @ Y))
    K = np.exp(-C / gamma)
...
def _rMSE(X, Rg, Rgt):
    ...
    return (np.linalg.norm(Rgt @ X.T - Rg @ X.T, 'fro') ** 2 /
            np.linalg.norm(Rgt @ X.T, 'fro') ** 2)
...
class HiWA:
    ...
    dim_red_method : object with a fit_transform() method or None
        Method to compute a low-d embedding of the source and target
        distributions.
        Defaults to PCA.

    parallelize : Boolean
        Whether to use the multiprocessing module to run ADMM iterations
        concurrently.
        Defaults to True
    ...
    def __init__(self, dim_red_method=PCA(n_components=2), normalize=True,
                 maxiter=300, tol=1e-1, mu=5e-3, shorn_maxiter=1000,
                 shorn_gamma=2e-1, sa_maxiter=100, sa_tol=1e-2,
                 sa_shorn_maxiter=150, sa_shorn_gamma=1e-1):
    ...
    def fit(self, X, X_labels, Y, Y_labels, **kwargs):
        ...
        Y : arraylike, shape (n_samples_y, n_features)
            Target dataset, of the same number of features as X, to rotate X
            to match.
        ...
        """
        # If not provided, compute transformations for source and target
        # datasets using the method specified during
        # initialization
        if self.normalize:
            X = _normal(X)
            Y = _normal(Y)
        X_transform = kwargs.get('X_transform', np.linalg.pinv(X) @
                                 self.dim_red_method.fit_transform(X))
        Y_transform = kwargs.get('Y_transform', np.linalg.pinv(Y) @
                                 self.dim_red_method.fit_transform(Y))
        self.Rgt = kwargs.get('Rgt', np.identity(X.shape[1]))

        # Initialization
        h_dim, num_clusters_x, num_clusters_y = (X.shape[1],
                                                 len(np.unique(X_labels)),
                                                 len(np.unique(Y_labels)))
        # Rg = np.identity(h_dim)
        Rg = _closed_form_rotation_solver(np.random.random((h_dim, h_dim)))
        P = np.full((num_clusters_x, num_clusters_y), 1 / (num_clusters_x *
                                                           num_clusters_y))
        ...
        for n in range(self.maxiter):
            ...
            for i in range(num_clusters_x):
                for j in range(num_clusters_y):
                    ...
                    R[:, :, i, j], _, C[i, j] = (
                        self._subspace_alignment_solver(
                            X_mbed[(X_labels == clust_ids_x[i]).squeeze(), :],
                            Y_mbed[(Y_labels == clust_ids_y[j]).squeeze(), :],
                            P[i, j], T))

            # Solve for P
            P, _ = _sinkhorn_clusters(p, q, C, self.shorn_gamma,
                                      self.shorn_maxiter)

            # Solve for global rotation, Rg
            Rg_prev = Rg
            Rg = _closed_form_rotation_solver(
                np.mean(np.reshape(R + L, [h_dim, h_dim, num_clusters_x *
                                           num_clusters_y], order='F'),
                        axis=2))
            ...
            if (((np.isnan(P).any() or diagnostics['Rg_norm'][n] <= self.tol)
                 and n >= 5)):
    ...
    def fit_transform(self, X, X_labels, Y, Y_labels, **kwargs):
        """Fit the model with X, learning a rotation to match X to Y, and
        apply the learned rotation to it.
    ...
    def _subspace_alignment_solver(self, X, Y, P, T):
        ...
        for i in range(self.sa_maxiter):
            ...
            # Solve Sinkhorn OT
            Q, dist = _sinkhorn(p, q, R @ X.T, Y.T, self.sa_shorn_gamma / P,
