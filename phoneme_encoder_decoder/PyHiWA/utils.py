...
def load_data(file_name):
    ...
    return (np.array(mat_dict['Tte']), np.array(mat_dict['Ttr']),
            np.array(mat_dict['Xte']), np.array(mat_dict['Xtr']),
            np.array(mat_dict['Yte']), np.array(mat_dict['Ytr']))
...
def plot_2d_clusters(X, labels):
    for i in np.unique(labels):
        plt.plot(X[labels == i, 0], X[labels == i, 1], linestyle='',
...
def eval_R2(X, Y):
    X = normal(X)
    Y = normal(Y)
    return (1 - np.mean(np.power(Y - X, 2), axis=0).sum() /
            np.var(Y, axis=0).sum())


def match_clustered_labels(X, X_labels, Y, Y_labels):
    """Find the optimal permutation of labels found by clustering, via
    1-nearest neighbors classification accuracy.
